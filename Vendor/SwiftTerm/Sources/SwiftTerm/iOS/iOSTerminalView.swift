...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    public var allowMouseReporting: Bool = true {
        didSet {
            guard didFinishSetup, terminal != nil else {
                return
            }
    ...
    private var configuredMinimumColumns = 0

    /// Keeps the terminal wider than the viewport so compact devices can pan
    /// across full-width terminal applications instead of forcing a reflow.
    public var minimumColumnCount: Int {
        get {
            configuredMinimumColumns
        }
        set {
            let value = max(0, newValue)
            guard value != configuredMinimumColumns else {
                return
            }
            configuredMinimumColumns = value
            guard didFinishSetup, terminal != nil else {
                return
            }
    ...
    private func updateMousePanGesture(
        mouseMode: Terminal.MouseMode? = nil
    ) {
        let activeMouseMode = mouseMode
            ?? (terminal != nil ? terminal.mouseMode : .off)
        if allowMouseReporting && activeMouseMode != .off {
    ...
    func getEffectiveWidth (size: CGSize) -> CGFloat
    {
        max(
            size.width,
            CGFloat(configuredMinimumColumns) * cellDimension.width
        )
    }
    ...
    func updateScroller ()
    {
        ...
        let offsetX = min(max(contentOffset.x, 0), maxContentOffsetX())
    ...
    private func maxContentOffsetX() -> CGFloat {
    ...
    private func resetManualScrollTracking() {
        ...
        let offsetX = min(max(contentOffset.x, 0), maxContentOffsetX())
        setContentOffsetFromTerminal(
            CGPoint(x: offsetX, y: bottomOffset)
        )
    }
    ...
    open override func accessibilityScroll(_ direction: UIAccessibilityScrollDirection) -> Bool {
        ...
        let pageWidth = max(bounds.width, cellDimension.width)
        ...
        case .up, .previous:
            targetOffset.y = max(0, contentOffset.y - pageHeight)
        case .right:
            targetOffset.x = min(
                maxContentOffsetX(),
        ...
        case .left:
            targetOffset.x = max(0, contentOffset.x - pageWidth)
        default:
            return super.accessibilityScroll(direction)
        }

        guard targetOffset != contentOffset else {
            return false
        }
    ...
    */
    ...
    open func mouseModeChanged(source: Terminal) {
        updateMousePanGesture(mouseMode: source.mouseMode)
    }
